<%-- 
    ...
    Created on : 10/03/2020, 01:26:00 PM
    Author     : Zaira
--%>
...
<html lang="es" ng-app="inicioMaterias" >
    <head>
        ...
        <title>Inicio Materia | SGAA</title>
    ...
    <body ng-controller="controladorMateria" ng-init="inicioMateria()">
        <!-- Pre-loader start -->
                                                        ...
                                                        <div class="col">
                                                            <div class="page-header-breadcrumb">
                                                                <ul class="breadcrumb-title">
                                                                    <a class="btn btn-success" href="registrarMateria" ><i class="icofont icofont-plus-circle"></i>Registrar</a>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                        ...
                                                        <div class="dt-responsive table-responsive">
                                                            <table id="simpletable" class="table table-striped table-bordered nowrap">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    <tr style="color: #000" ng-repeat ="materia in listaMaterias" style="color: white">

                                                                        <td>{{materia.identificador}}</td>
                                                                        <td>{{materia.nombre}}</td>
                                                                        <td>{{materia.desc}}</td>
                                                                        <td>{{materia.carrera.nombre}}</td>
                                                                        <td>{{materia.numero_cuatri.numero}}</td>
                                                                        <td> <s:form action="consultaIndividualMateria">
                                                                                <input type="hidden" ng-model="materia.id_materia" name="bean.materia.id_materia" value="{{materia.id_materia}}" />
                                                                                <!-- Button trigger modal -->
                                                                                {{id = materia.id_materia}}
                                                                                <button type="button" ng-click="inicioDocente(id)"class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter{{materia.id_materia}}">
                                                                                    Asignar
                                                                                </button>
                                                                            </s:form>
                                                                            <!-- Modal -->
                                                                            <div class="modal fade" id="exampleModalCenter{{materia.id_materia}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                                                    <div class="modal-content">
                                                                                        <div class="modal-header">
                                                                                            <h5 class="modal-title" id="exampleModalLongTitle">Asignar Docente</h5>
                                                                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                                                <span aria-hidden="true">&times;</span>
                                                                                            </button>
                                                                                        </div>
                                                                                        <form id="" method="post" action="<%=context%>/registrarMateriaDocente">
                                                                                            <div class="modal-body">
                                                                                                <input type="hidden" name="bean.materia.id_materia" value="{{materia.id_materia}}" />
                                                                                                <div class="form-group row">
                                                                                                    <label class="col-sm-2 col-form-label">Docente<span style="color: red">*</span></label>
                                                                                                    <div class="col-sm-10">

                                                                                                        <select class="custom-select col-12" id="docente" required  id="bean.docente.id_docent" name="bean.docente.id_docent" >
                                                                                                            <option selected="true" value="">Seleccionar docente...</option>
                                                                                                            <option ng-repeat="docent in listaDocentes" value="{{docent.id_docent}}">{{docent.nombre + ' ' + docent.primer_apellido + ' ' + docent.segundo_apellido}}</option>
                                                                        ...
                                                                        <td>   
                                                                            <s:form action="consultaIndividualMateria">
                                                                                <s:hidden ng-model="materia.id_materia" name="bean.id_materia" value="{{materia.id_materia}}" />
                                                                                <button  class="btn btn-warning btn-icon" rype="submit"> <i class="icofont icofont-edit"></i> </button>
                                                                            </s:form>
                                                                        </td> 
                                                                        <td>    
                                                                            <div ng-if="materia.estado.id_estado === 2">
                                                                                <form action="estadoMateria" method="post" id="inactivo{{materia.id_materia}}">
                                                                                    <input name="bean.estado.id_estado" type="hidden" value="1">
                                                                                    <input type="hidden" value="{{materia.id_materia}}" name="bean.id_materia">
                                                                                    <a href="javascript:void(0)" class="btn btn-success" 
                                                                                       ng-click="confirmarEstadoMateria(materia.id_materia)">
                                                                                        <i class="ti-check"></i>
                                                                                    </a>
                                                                                </form>
                                                                            </div>
                                                                            <div ng-if="materia.estado.id_estado === 1">
                                                                                <form action="estadoMateria" method="post" id="inactivo{{materia.id_materia}}">
                                                                                    <input name="bean.estado.id_estado" type="hidden" value="2">
                                                                                    <input type="hidden" value="{{materia.id_materia}}" name="bean.id_materia">
